...
class DataPreprocessor:
    ...
    def check_data_in_s3(self):
        """Check if both train.csv and test.csv files are present in the specified S3 path."""
        try:
    ...
    def load_data(self):
        try:
            ...
            reviews = load_dataset("McAuley-Lab/Amazon-Reviews-2023", "raw_review_All_Beauty", trust_remote_code=True).get('full')
    ...
    def run(self):
        if not self.check_data_in_s3():
            ...
            combined_df = self.preprocess_data(metadata, reviews)
            file_paths = self.split_and_save_data(combined_df)
